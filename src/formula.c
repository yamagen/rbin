...
//
/*
* The following functions are used to calculate the bin width and the number of bins for a histogram.
* The bin width is the width of each bin in the histogram, and the number of bins is the number of bins in the histogram.
* The bin width is calculated using the Wand bin width formula, and the number of bins is calculated using the Freedman-Diaconis rule.
* The Wand bin width formula is used to calculate the bin width based on the standard deviation of the data and the number of data points.
  * The Freedman-Diaconis rule is used to calculate the number of bins based on the interquartile range of the data and the number of data points.
* The bin width and the number of bins are used to create a histogram that accurately represents the data.
* In statistics, the Freedman芯iaconis rule can be used to select the width of the bins to be used in a histogram.[1] It is named after David A. Freedman and Persi Diaconis. 
*/
...
/* Freedman-Diaconis bin width */
double
fd_bin_width(double *data, size_t n)
{
  double *sorted_data;
  double iqr;
  double h;
